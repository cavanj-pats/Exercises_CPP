//leet 236 Lowest Common Ancestor
//
...
public:
 ...
 /*       //this is some brilliant guys code on LeetCode discussion
...
//};
    ...
    void inOrderVectorFill(TreeNode* root, vector<TreeNode*> &vValues, TreeNode* n ) {
        if (root ){
            if(root->val != n->val){
                vValues.push_back(root);
                inOrderVectorFill(root->left, vValues, n );
                inOrderVectorFill(root->right, vValues, n);   
            } else{
                vValues.push_back(root);
            }
...
int main()
{
    ...
    TreeNode* root = node;
    Solution sol;
    cout << "Lowest Common Node of 15, 17 should be 2: "<<  (sol.lowestCommonAncestor(root, root->left, root->left->right->right))->val << endl;

...
#include "std_lib_facilities.h"  //for Visual Studio
...
double primary()
{
	...
	{	double d = expression();  //evaluate the expression which will call primary again
	...
	case '-':
		return -primary();  //if '-' return minus primary
	case number:
...
double term()
{
	double left = primary();  //evaluate * or /
	while (true) {
...
double expression()
{
	double left = term();  //send to term to ensure * or / or expression in ( ) evaluated first
...
double declaration()
{
	...
	double d = expression();  // if it gets here it is a correctly formatted expression that 
	                          // needs to be returned
	names.push_back(Variable(name, d));  //  add this variable to the variable table vector
	return d;
}

double pwr_expression()
{
	double t1 = expression(); //first term in power function
	int t2 = narrow_cast <int> (expression());  //second term in power function
	return pow(t1, t2);
...
double statement()
{
	...
	switch (t.kind) {
	  ...
	  case power:
	  {
		  ...
		  is_pwr = true;
		  double d = pwr_expression();  //should return value of expression up to ','
		  return d;
	  ...
	  default:
	  {
		...
		return expression();  //if not let put it back and call expression
	  }
	}
}
...
void calculate()
{
	while (true) try {
		...
		cout << result << statement() << endl;   //determine what statement we have
	}
